...
export function NewOrderStatusSheet() {
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: (values: z.infer<typeof formSchema>) => {
      return privateInstance.post('/tenant/order-status', values)
    },
    onSuccess: (response) => {
      if (response.status === 200 || response.status === 201) {
        toast.success("Status de pedido cadastrado com sucesso!")
        closeSheet()
        queryClient.invalidateQueries({ queryKey: ['order-status'] })
      } else {
        const errorData = (response.data as any)
        toast.error(errorData?.title || "Erro ao cadastrar status de pedido", {
          description: errorData?.detail || 'Não foi possível cadastrar o status de pedido.'
        })
      }
    },
    onError: (error: any) => {
      ...
      <SheetContent>
        <Form {...form}>
          <form onSubmit={(e) => { e.stopPropagation(); form.handleSubmit(onSubmit)(e); }} className="flex flex-col h-full">
            <SheetHeader>
              <SheetTitle>Cadastro de status de pedido</SheetTitle>
              <SheetDescription>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Digite o nome do status de pedido..." {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
