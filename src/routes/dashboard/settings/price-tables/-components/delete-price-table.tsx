...
export function DeletePriceTable({ id, onDeleted }: { id: number, onDeleted?: () => void }) {
  const [open, setOpen] = useState(false)
  const queryClient = useQueryClient()
  const { isPending, mutateAsync } = useMutation({
    mutationFn: async () => {
      const response = await privateInstance.delete(`/tenant/price-tables/${id}`)
      if (response.status !== 200) throw new Error('Erro ao excluir tabela de preço')
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Excluir tabela de preço</DialogTitle>
