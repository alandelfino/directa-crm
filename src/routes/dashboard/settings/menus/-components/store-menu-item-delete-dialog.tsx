...
export function StoreMenuItemDeleteDialog({ storeMenuId, itemId, onDeleted }: { storeMenuId: number, itemId: number, onDeleted?: () => void }) {
  const [open, setOpen] = useState(false)
  const { isPending: deleting, mutate: deleteItem } = useMutation({
    mutationFn: async () => {
      const response = await privateInstance.delete(`/tenant/store-menus/${storeMenuId}/items/${itemId}`)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button size="sm" onClick={() => deleteItem()} disabled={deleting}>
            {deleting ? <Loader className='animate-spin size-[0.85rem]' /> : 'Confirmar'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
