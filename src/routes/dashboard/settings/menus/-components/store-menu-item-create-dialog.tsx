...
export function StoreMenuItemCreateDialog({
  storeMenuId,
  parentOptions,
  onCreated,
}: {
  ...
  const { isPending: creating, mutate: createItem } = useMutation({
    mutationFn: async (values: z.infer<typeof schema>) => {
      ...
      const response = await privateInstance.post(`/tenant/store-menus/${storeMenuId}/items`, payload)
  ...
  return (
    <Sheet open={open} onOpenChange={(o) => {
      ...
      <SheetContent className='w-lg sm:max-w-[540px] p-0'>
        <Form {...form}>
          <form onSubmit={form.handleSubmit((values) => createItem(values))} className='flex flex-col h-full'>
            <SheetHeader className='px-4 py-4'>
              <SheetTitle>Novo item</SheetTitle>
              <SheetDescription>Preencha os dados e salve.</SheetDescription>
            </SheetHeader>
            ...
            <div className='flex flex-col flex-1 overflow-hidden'>
              <div className='flex-1 grid auto-rows-min gap-6 px-4 py-4 overflow-y-auto'>
                ...
                <FormField control={form.control as any} name='parentId' render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Select value={field.value ? String(field.value) : 'root'} onValueChange={(v) => field.onChange(v === 'root' ? undefined : Number(v))} disabled={creating}>
                        <SelectTrigger>
                ...
                <FormField control={form.control as any} name='active' render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-md border p-3">
                    ...
                    <FormControl>
                      <Switch checked={field.value} onCheckedChange={field.onChange} disabled={creating} />
                    </FormControl>
                  </FormItem>
                )} />
              </div>
              ...
              <div className='border-t p-4'>
                <div className='grid grid-cols-2 gap-4'>
                  ...
                  <Button size="sm" type="submit" disabled={creating} className='w-full'>
                    {creating ? <Loader className='animate-spin size-[0.85rem]' /> : 'Salvar'}
                  </Button>
                </div>
              </div>
            </div>
